...
Calls the Gemini API (gemini-2.0-flash) to generate a structured
...
def get_briefing(country_iso: str):
    ...
    - On cache miss: fetches tension + recent headlines from MongoDB,
      calls Gemini 2.0 Flash, parses structured JSON, stores result.
    ...
    try:
        ...
        traceback.print_exc()  # DEBUG: show real error in uvicorn terminal
        raise HTTPException(
            status_code=502,
